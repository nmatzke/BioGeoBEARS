...
# wd = "/drives/GDrive/z_help/Furnariidae_Bio_Geo/BSM_small/M0/"
# setwd(wd)
...
wd = slashslash(paste0(addslash(extdata_dir), "/examples/BSM_3taxa/M0/"))
setwd(wd)
...
runslow = TRUE
resfn = "3taxa_DIVALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
    {
    res = bears_optim_run(BioGeoBEARS_run_object)
    res    
...
resfn = "3taxa_DIVALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
    {
    #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = "3taxa_DIVALIKE_vs_DIVALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on 3taxa M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on 3taxa M0_unconstrained"
...
runslow = TRUE
resfn = "3taxa_BAYAREALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
    {
    res = bears_optim_run(BioGeoBEARS_run_object)
    res    
...
resfn = "3taxa_BAYAREALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
    {
    res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "3taxa_BAYAREALIKE_vs_BAYAREALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on 3taxa M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on 3taxa M0_unconstrained"
...
# Get the list of states, areanames, and ranges_list (names of the ranges)
# Specify a results object
res = resBAYAREALIKE
clado_events_table = clado_events_tables[[1]]
ana_events_table = ana_events_tables[[1]]
...
#######################################################
# Convert BioGeoBEARS BSM output to phytools - multi-Simmap
#######################################################
...
set.seed(seed=54321)
tr3 = make.simmap(tree=tr, x=tipdata, model="ER", nsim=3)
tr3
tr3$mapped.edge
